...
function getLastDigit(value) {
  ...
  if (value >= 0 && value < 100) return value % 10;
  return value % 100;
}
...
function getSumOfDigits(num) {
  ...
  let sum = 0;
  for (let i = 0; i < str.length; i += 1) sum += +str[i];
  return sum;
}
...
function toPrecision(number, precision) {
  const arrLength = String(number).split('').length;
  if (arrLength < precision) return number.toFixed(precision - arrLength);
...
function getCountOfOddNumbers(number) {
  ...
  for (let i = 0; i <= Math.abs(number); i += 1) if (i % 2 !== 0) count += 1;
